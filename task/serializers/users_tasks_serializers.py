...
class UsersTasksCreateAndAssignSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data:dict, user):
        try:
            ...
            self._assign_task(task, assigned_to)
            return True
        except:
            pass

    def _create_task(self, task_data:dict, user):
        try:
    ...
    def _assign_task(self, task, assigned_to_pk):
        try:
            ...
            users_task = UsersTasks.create_factory(commit=False, task=task, assigned_to=assigned_to_user)
            users_task.save()
        except Exception as exception:
            raise DBOperationFailed(detail={"detail": _(exception.__str__())})
